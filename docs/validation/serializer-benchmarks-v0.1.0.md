...
Its post-run sleep-event audit passed; the accepted Protobuf 3 interval also had no recorded sleep events.
...
Protobuf 3 text construction/serialization results have broad intervals, so their point estimates are insufficient for an acceptance claim.
...
Even the non-Protobuf lanes were rerun in each profile, so their timing differences include host and run variation.
